; Bonero Fluorine Fermi GUI Wallet Installer for Windows
; Copyright (c) 2017-2024, The Bonero Project
; See LICENSE
#define GuiVersion GetFileVersion("bin\bonero-wallet-gui.exe")

[Setup]
AppName=Bonero GUI Wallet
...
DefaultDirName={commonpf}\Bonero GUI Wallet
DefaultGroupName=Bonero GUI Wallet
UninstallDisplayIcon={app}\bonero-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The Bonero Developer Community
AppPublisherURL=https://getbonero.org
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the Bonero files can be copied to a place where they have at least
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Bonero.

Source: {#file AddBackslash(SourcePath) + "ReadMe.htm"}; DestDir: "{app}"; DestName: "ReadMe.htm"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy
Source: "LICENSE"; DestDir: "{app}"; Flags: ignoreversion

; Bonero GUI wallet exe and guide
Source: "bin\bonero-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\bonero-gui-wallet-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; Bonero CLI wallet
Source: "bin\extras\bonero-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\bonero-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; Bonero wallet RPC interface implementation
Source: "bin\extras\bonero-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Bonero daemon
Source: "bin\bonerod.exe"; DestDir: "{app}"; Flags: ignoreversion

; Bonero daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "bonero-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Bonero blockchain utilities
Source: "bin\extras\bonero-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\bonero-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\bonero-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\bonero-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\bonero-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\bonero-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\bonero-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\bonero-blockchain-prune-known-spent-data.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\bonero-blockchain-prune.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\bonero-blockchain-stats.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\extras\bonero-gen-ssl-cert.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\bonero-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitbonero');
  s := 'The default folder to store the Bonero blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\bonero-project\bonero-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Bonero default location
...
begin
  Result := BlockChainDir('') + '\bitbonero.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "bonero-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Bonero daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\bonerod.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\bonero-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Bonero GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\bonero-wallet-gui.exe";
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\bonero-gui-wallet-guide.pdf"; IconFilename: "{app}\bonero-wallet-gui.exe"
...
Name: "{group}\Utilities\Bonero Daemon"; Filename: "{app}\bonerod.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\bonero-wallet-cli.exe"; WorkingDir: "{userdocs}\Bonero\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: """{userdocs}\Bonero\wallets"""
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: """{userappdata}\bonero-wallet-gui\bonero-wallet-gui.log"""
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\bonero-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM bonerod.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\bonero-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\bonero-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\bonero-project\bonero-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\bonero-project\bonero-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "bonero:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "bonero://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "bonero"; ValueType: "string"; ValueData: "URL:Bonero Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "bonero"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "bonero\DefaultIcon"; ValueType: "string"; ValueData: "{app}\bonero-wallet-gui.exe,0"
Root: HKCR; Subkey: "bonero\shell\open\command"; ValueType: "string"; ValueData: """{app}\bonero-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "boneroseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "boneroseed://a.b.c.d:port"
Root: HKCR; Subkey: "boneroseed"; ValueType: "string"; ValueData: "URL:Bonero Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "boneroseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "boneroseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\bonero-wallet-gui.exe,0"
Root: HKCR; Subkey: "boneroseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\bonero-wallet-gui.exe"" ""%1"""
